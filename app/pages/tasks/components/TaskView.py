...
class TaskViewComponent:
    def __init__(self, task_data, on_save=None, on_cancel=None):
        self.task = task_data.copy()  # Create a copy to track changes

        # Format task_comments from DynamoDB format if it exists
        if "task_comments" in self.task:
            raw_comments = self.task["task_comments"].get("L", [])
            self.task["task_comments"] = []
            for comment in raw_comments:
                comment_map = comment.get("M", {})
                if comment_map:  # Only add if we have a valid comment map
                    self.task["task_comments"].append(
                        {
    ...
    def _add_comment(self):
        ...
        try:
            ...
            comment = {
                "comment_id": {"S": str(len(self.task.get("task_comments", [])))},
                "user_id": {"S": user_id},
            ...
            if "task_comments" not in self.task:
                self.task["task_comments"] = []

            update_expr = "SET task_comments = list_append(if_not_exists(task_comments, :empty_list), :new_comment)"
            expr_values = {
            ...
            self.task["task_comments"].append(
                {
                    "comment_id": str(len(self.task.get("task_comments", []))),
                    "user_id": user_id,
        ...
        with ui.column().classes("w-full max-w-3xl mx-auto flex-grow"):
            ...
            with self.scroll_area:
                ...
                with self.message_container:
                    comments = self.task.get("task_comments", [])
    ...
    def render(self):
        with ui.card().style(
            self.style_manager.get_style("task_view.container")
        ).classes("w-full"):
            with ui.card().style("width:80%; height:70%;"):
                with ui.row().classes("w-full justify-between gap-2 mb-4"):
                    with ui.row().classes(
                        ...
                        ui.input(
                            "Title", value=self.task.get("task_name", "")
                        ).bind_value_to(self.task, "task_name").classes("w-3/5").props(
                            "outlined"
                        )
                ...
                with ui.grid(columns=5).classes("w-full h-full gap-4"):
                    with ui.column().classes("col-span-3"):
                        ui.textarea(
                            "Description",
                            value=self.task.get("task_description", ""),
                        ).style(
                            self.style_manager.get_style("task_view.input")
                        ).bind_value_to(
                        ...
                        with ui.row().classes("w-full gap-4"):
                            ui.select(
                                ...
                                value=self.task.get("task_priority", 1),
                                label="Priority",
                            ).style(
                                self.style_manager.get_style("task_view.input")
                            ).bind_value_to(
                                self.task, "task_priority"
                            )
                            ui.select(
                                ["Pending", "In Progress", "Completed"],
                                value=self.task.get("task_status", "Pending"),
                                label="Status",
                            ).style(
                                self.style_manager.get_style("task_view.input")
                            ).bind_value_to(
                        ...
                        with ui.row().classes("w-full h-full items-end"):
                            ui.button("Update", on_click=self._handle_save).style(
                                self.style_manager.get_style("task_view.button")
                            )
