...
class ListViewComponent:
    ...
    def _format_grid_data(self, tickets):
        return [
            {
                "title": ticket.get("ticket_title", {}).get("S", ""),
                "description": truncate_text(
                    ticket.get("ticket_description", {}).get("S", ""), 50
                ),
                "due_date": format_date(ticket.get("ticket_due_date", {}).get("S", "")),
                "status": ticket.get("ticket_status", {}).get("S", ""),
                "priority": format_priority(
                    int(ticket.get("ticket_priority", {}).get("N", "1"))
                ),
                "raw_data": {
                    "ticket_title": ticket.get("ticket_title", {}).get("S", ""),
                    "ticket_description": ticket.get("ticket_description", {}).get(
                        "S", ""
                    ),
                    "ticket_due_date": ticket.get("ticket_due_date", {}).get("S", ""),
                    "ticket_status": " ".join(
                        word.capitalize()
                        for word in ticket.get("ticket_status", {})
                        .get("S", "pending")
                        .split()
                    ),
                    "ticket_priority": int(
                        ticket.get("ticket_priority", {}).get("N", "1")
                    ),
