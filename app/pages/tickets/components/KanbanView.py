...
class KanbanViewComponent:
    ...
    def _handle_ticket_drop(self, ticket: dict, status: str) -> None:
        try:
            ...
            ui.notify(f"Ticket moved to {formatted_status}", type="positive")
    ...
    def render(self):
        tickets = self.state["tickets"].get("Items", [])
            ...
            with ui.row().style(self.style_manager.get_style("kanban_view.container")):
                with KanbanColumn("Pending", on_drop=self._handle_ticket_drop):
                    for ticket in tickets:
                        if (
                            ticket.get("ticket_status", {}).get("S", "").lower()
                            == "pending"
                        ):
                            formatted_ticket = {
                                "ticket_title": ticket.get("ticket_title", {}).get(
                                    "S", ""
                                ),
                                "ticket_description": ticket.get(
                                    "ticket_description", {}
                                ).get("S", ""),
                                "ticket_due_date": ticket.get(
                                    "ticket_due_date", {}
                                ).get("S", ""),
                                "ticket_status": ticket.get("ticket_status", {}).get(
                                    "S", ""
                                ),
