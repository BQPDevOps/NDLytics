...
class CognitoAdapter:
    def __init__(
        ...
        self.region = region
        self.dynamodb_sessions_table_name = dynamodb_sessions_table_name
    ...
    def _onload(self):
        ...
        self.dynamodb_table = self.dynamodb_client.Table(
            self.dynamodb_sessions_table_name
        )
    ...
    def get_user_groups(self, provided_token=None):
        if provided_token is None:
            ...
            token = AccessToken(encoded_token=access_token)
            decoded_token = token.get_decoded_token()
        else:
            token = AccessToken(encoded_token=provided_token)
            decoded_token = token.get_decoded_token()
        try:
    ...
    def verify_session(self, username, session_id):
        ...
        try:
            response = self.dynamodb_table.get_item(
                Key={"uuid": user_id, "session_id": session_id}
            )
    ...
    def create_user(
        ...
        company_id,
        temporary_password,
        permissions_role=None,
        permissions_granular=None,
    ...
    def list_permission_groups(self):
